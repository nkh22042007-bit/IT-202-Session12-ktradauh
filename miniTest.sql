...
SELECT std.StudentID, std.FullName, d.DeptName
FROM Student std
JOIN Department d ON std.DeptID = d.DeptID;

SELECT * FROM ViewStudentBasic;

ALTER TABLE Student ADD INDEX idxFullName (FullName);
...
SET @myScore = 12.5;
CALL UpdateScoreITDB('S00001', @myScore);
SELECT @myScore AS NewScore_After_Proc;
